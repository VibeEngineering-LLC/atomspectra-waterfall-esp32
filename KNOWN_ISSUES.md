...
### WiFi STA: modem sleep отключён (`WIFI_PS_NONE`)

По умолчанию ESP-IDF 5.4 STA держит **MIN_MODEM** power save — ICMP/HTTP RTT на
idle-плате гуляет **20–400+ ms** почти без потерь. В `wifi_manager.c` после
