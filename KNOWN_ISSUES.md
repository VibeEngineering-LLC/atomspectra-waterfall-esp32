...
### AWF-2a: TCP-мост не считается «работой с платой»

**Статус:** ограничение (v1.2.24)

Настройка «Переходить в полевую точку доступа при потере Wi-Fi» (по умолчанию выключена)
при включении возвращает плату к роутеру, только если пользователь не работал с веб-страницей
10 минут. Работа через ПК-программу (AtomSpectra/BecqMoni) по TCP-мосту в это не засчитывается.
Если настройка включена и вы работаете через TCP-мост, пока плата в полевой точке, связь может
оборваться перезагрузкой раньше, чем вы закончите. Обход: держите настройку выключенной
(по умолчанию) либо периодически открывайте веб-страницу платы.

### AWF-3: редкие случаи двойного счёта или задержки при сбросе прибора

**Статус:** ограничение (v1.2.24)

«База» (см. раздел «Спектр после отключения питания» в README) в норме сохраняет спектр без
потерь. В двух редких сочетаниях возможна неточность: (1) если после реального сброса прибора
пакет времени приходит с задержкой больше нескольких секунд ИЛИ отставание держится первые
секунды новой сессии — счёт в этом узком окне может однократно удвоиться; (2) при одновременной
записи двух сохранений базы подряд на flash может на короткое время остаться версия старее той,
что в памяти (переживёт следующую запись). Оба случая не теряют данные необратимо и не влияют
на обычную работу.
...
### «Перезагрузить прибор» стирает накопленный спектр — ИСПРАВЛЕНО (v1.2.24)

Прибор при перезагрузке (и при любом другом сбросе — обрыв питания, зависание) обнуляет свою
гистограмму. С v1.2.24 шлюз это не принимает молча: накопленный спектр сохраняется в «базу»,
и показ становится «база + текущий набор прибора». Кнопка «Перезагрузить прибор» больше не
теряет данные так же, как раньше не терял их обрыв питания (issue AWF-1/AWF-3). Резервные
снимки (issue #52) остаются дополнительной защитой, не единственной.
