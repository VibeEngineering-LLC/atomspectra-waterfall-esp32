...
### P-014: открытый Web UI снижает полноту измерения (частично исправлено в v1.2.11)

**Статус:** частично исправлено в `v1.2.11` · остаток известен, архитектурные меры запланированы.

Обслуживание HTTP конкурирует с приёмом данных от прибора за CPU/шину/флеш: пока плата
отвечает браузеру, USB-кадры могут теряться, и секундные свипы спектра бракуются по CRC —
**тихо**, без ошибок в интерфейсе. Замер до фиксов (та же методика, живая плата): в покое
потерь 0 %, открытая вкладка «Водопад» — 9,4 % потерянных секунд, «Система» — 25,8 %,
6 параллельных клиентов — 95,6 %.

В `v1.2.11` исправлены два главных виновника: подсчёт занятого места LittleFS обходил весь
раздел на каждый `GET /api/system` (теперь кэш 30 с) и `GET /api/device` копировал 32 КБ
спектра под локом приёма ради четырёх полей метаданных (теперь ~200 Б). После: «Система»
2,8 %, «Водопад» 1,1 %.

**Остаток:** `GET /api/waterfall/segments` читает шапку каждого файла с флеш (потери растут с
числом сегментов); шторм многих одновременных клиентов по-прежнему разрушителен. Наблюдать
свою плату: страница Сервис → «Канал USB» → «Спектров развалилось» — рост при работе с UI
и есть эта проблема.

**Рекомендация до полного исправления:** во время ответственных длительных измерений не
держать вкладки Web UI открытыми постоянно (особенно «Система» — она самая тяжёлая);
заглянул — закрыл. Сборщик `wf-recorder` с разумным интервалом безопасен.
