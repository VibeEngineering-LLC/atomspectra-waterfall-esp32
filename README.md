...
**🌲 Outdoor (полевой AP)** — плата **сама становится точкой доступа**, роутер не нужен.
Для работы в поле, где есть только телефон:

- SSID `AtomSpectra-Outdoor`, пароль `atomspectra` (WPA2), IP `192.168.4.1`;
- телефон подключается к этой сети → **captive-portal** сам открывает Web UI
  (как «вход в сеть» у публичного WiFi);
- полный интерфейс: живой спектр, водопад, экспорт, управление прибором;
- данные водопада копятся на flash платы — забираются дома в режиме Indoor.

**Как войти в Outdoor:**

1. **Переключателем** Indoor→Outdoor в шапке стартовой страницы (плата перезагрузится);
2. **Автоматически** — если домашняя сеть недоступна ≥ 90 с, плата сама поднимает
   полевой AP (одноразово; следующий ребут снова пробует домашнюю сеть);
3. **С новой платы** — в setup-портале кнопка «Полевой режим (Outdoor)».

Возврат в Indoor — тем же переключателем.
