...
платы → Сеть, `/api/boot-config`) — по умолчанию включена (как описано выше). Выключенная,
она отменяет автоматический fallback ТОЛЬКО пока плата хоть раз подключилась к домашней
сети в текущей загрузке: реконнект идёт бесконечно с нарастающей паузой (потолок 60 с),
в Field AP плата не уходит. Страховка сохраняется в любом случае: если с момента
включения/перезагрузки Wi-Fi ни разу не заработал 5 минут подряд, плата всё равно поднимет
полевой AP — иначе после смены пароля или роутера до неё было бы не достучаться.
...
- Мост прозрачно пересылает сырой поток прибора: ПК-клиент видит только набор самого анализатора с момента его последнего сброса, БЕЗ базы Web UI (AWF-3) — после обрыва питания или -rst счётчик в BecqMoni/AtomSpectra начнётся заново, даже если веб-интерфейс уже показывает непрерывный спектр.
