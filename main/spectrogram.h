...
typedef struct {
    ...
    uint32_t seg_dropped;    // #REC-11-A1: сегментов удалено кольцом ЗА ТЕКУЩУЮ СЕССИЮ
                             // (обнуляется при каждом старте — см. seg_evicted ниже)
    // #FW-56: накопительный счётчик вытеснений кольцом, переживает ребут (NVS).
    // Зачем отдельно: seg_dropped сбрасывается boot-реконсиляцией, и картина
    // «сегментов стало меньше, а счётчик 0» читается как тихая потеря данных.
    // Именно так родился ложный инцидент P-015: кольцо работало штатно, но
    // доказать это по счётчику было нечем. Пишется в NVS В МОМЕНТ вытеснения:
    // откладывать до открытия следующего сегмента нельзя — вытеснение идёт по
    // prep-триггеру в середине сегмента, и ребут в этом окне уносил счётчик.
