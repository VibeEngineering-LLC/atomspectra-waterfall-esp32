...
typedef struct {
    ...
    // #FW-57 (P-016 «Побочная находка», 2026-08-19): до этого фикса ОДНО поле
    // seg_dropped считало два разных события (реальная потеря в seg_reconcile
    // И безопасное вытеснение кольцом в make_room) — рост счётчика было нельзя
    // интерпретировать однозначно. Теперь разведено на два поля с непересекающимся
    // смыслом:
    uint32_t seg_lost;       // РЕАЛЬНАЯ потеря данных: boot-реконсиляция (seg_reconcile)
                             // нашла сегмент с size=0, хотя строки физически могли быть
                             // записаны на flash (inode не обновился без fsync — см. P-016).
                             // НЕ включает штатное вытеснение кольцом (см. seg_evicted).
                             // Накопительный, переживает ребут (NVS "seg_lost"); сбрасывается
                             // только явной очисткой (spectrogram_clear) — как и seg_evicted,
                             // молча обнулять ребутом нельзя (тот же урок, что и P-015).
    // #FW-56: накопительный счётчик БЕЗОПАСНЫХ вытеснений кольцом keep-last
    // (make_room — сегмент уже был скачан или устарел), переживает ребут (NVS).
    // Зачем отдельно от seg_lost: раньше «сегментов стало меньше, а счётчик 0»
    // читалось как тихая потеря данных — так родился ложный инцидент P-015.
    // Пишется в NVS В МОМЕНТ вытеснения: откладывать до открытия следующего
    // сегмента нельзя — вытеснение идёт по prep-триггеру в середине сегмента,
    // и ребут в этом окне уносил счётчик.
    uint32_t seg_evicted;
    time_t   started_at;
...
/* spectrogram_seg_open_index() удалена в #FW-61 — см. комментарий на месте определения
   в spectrogram.c. Открытый сегмент определяется по реестру: finalized == false. */
...
// (раньше — opendir/stat/fopen на каждый запрос: max 1550 мс при четырёх файлах).
// Порядок — по возрастанию idx. Возвращает число заполненных записей (≤ cap).
// Ёмкость реестра. Считать надо по МИНИМАЛЬНОМУ сегменту, а не по максимальному: сегмент
// закрывается по WF_SEG_MAX_ROWS строкам ИЛИ по возрасту (WF_SEG_MAX_AGE_SEC) — что раньше.
// Поэтому частая запись даёт МАЛО крупных файлов, а редкая — МНОГО мелких:
//   interval=5   -> закрытие по 64 строкам  -> ~1,09 МБ -> ~12 файлов
//   interval=180 -> закрытие по возрасту    -> ~102 КБ  -> ~157 файлов (на плате видели 45
//                                                          при трети заполнения)
//   interval>300 -> одна строка на сегмент  -> ~53 КБ   -> предел (см. ниже)
//
// Фактический предел (все величины сверены с платой и partitions.csv, 2026-08-20):
//   раздел storage 0xCF0000                        = 13 565 952 Б
//   минус WF_FLASH_RESERVE (кольцо вытесняет ДО    = -1 048 576 Б
//     полного заполнения, см. make_room)
//   доступно под сегменты                          = 12 517 376 Б
//   минимальный непустой файл 4104+32768+16410     =     53 282 Б
//   он же с блочностью LittleFS (14 блоков × 4096) =     57 344 Б
//   => 12 517 376 / 57 344                         =        218 сегментов
//
// 256 покрывает предел с запасом 17 % и остаётся степенью двойки. Цена: запись 32 Б
// (26 Б полей + выравнивание до кратного 8 из-за int64_t), итого 256×32 = 8 КБ — столько же
// временно берётся в куче на время формирования листинга. Держать эту цифру в соответствии
// с составом структуры: добавление поля молча увеличивает и реестр, и буфер обработчика.
// Не считать «по максимальному размеру сегмента»: первая редакция так получила 24 —
// в девять раз меньше факта, реестр молча терял бы большую часть листинга.
// Не забывать вычесть резерв и учесть блочность: без них выходит 254 (завышение на 16 %).
#define WF_SEG_REG_MAX 256
typedef struct {
    int64_t  started_at;
    uint32_t idx;
    uint32_t seg_seq;
    uint32_t rows;
    // #FW-61: ФАКТИЧЕСКИЙ размер файла (stat), а не вычисленный обратной формулой.
    // Формула `rows*WF_ROW_STRIDE + WF_SEG_HEADER + WF_BASELINE_BYTES` верна только для
    // текущего формата v5: на flash легитимно живут сегменты v1..v4, пережившие OTA, у
    // них другой stride и нет baseline-секции. Для v2-файла из 64 строк формула завышает
    // размер на 34 304 Б, и приёмник (сверяет длину скачанного с заявленной) отвергает
    // сегмент КАЖДЫЙ проход, никогда не подтверждает — файл висит, пока кольцо не сотрёт.
    uint32_t bytes;
    bool     finalized;
    bool     valid;
} wf_seg_reg_t;
