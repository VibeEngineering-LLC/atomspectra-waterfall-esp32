...
2. **Сама прошивка** — сегменты `/storage/wf/seg_NNNNN.aswf` (#REC-11-A1), формат
   **v2**: после 16384 Б спектра каждая строка несёт **2 Б реальной длительности**
   ...
   `saved_rows`/`saved_at` идут **первыми фиксированной ширины** — их прошивка
...
`serial`/`calibration` присутствуют только если прибор их сообщил; у сегмента
`saved_rows=0` означает, что он ещё не финализирован.
...
- Ролловер сегмента — **без мёртвого окна**: шапка нового сегмента преаллоцирована
  (`WF_HDR_RESERVE`), строки пишутся сразу, патчится только финализация.
