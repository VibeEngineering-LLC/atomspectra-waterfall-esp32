...
| Переживание ребута | на boot `spectrogram_restore()` сверяет `/storage/wf`: патчит счётчики у недописанных файлов, удаляет пустые огрызки, восстанавливает индекс; **если запись была активна — продолжает в НОВЫЙ сегмент** (mid-segment append не делается, каждый файл остаётся валидным) |
| Flash заполнен | **кольцо keep-last**: затирается самый старый ещё не отправленный сегмент; `flash_full=true`, растёт `seg_dropped` |
| Открытый сегмент | его шапка ещё не пропатчена (`saved_rows=0`) → в `/segments` помечен `finalized:false`; забирать в браузер до finalize не нужно |
...
- `finalized:false` — сегмент сейчас открыт (шапка ещё не пропатчена) — забирать не нужно;
...
2. **Сама прошивка** — сегменты `/storage/wf/seg_NNNNN.aswf` (#REC-11-A1). Тот же
   каркас, но `header_len` **фиксирован = 4096** (`WF_HDR_RESERVE`; JSON добивается
   пробелами, payload всегда с offset **4104**), а счётчики называются
   `saved_rows`/`saved_at` и идут **первыми фиксированной ширины** — их прошивка
   патчит по месту (offset 22 и 44) на финализации, не переписывая все 4 КБ:
...
Читатель, который берёт `header_len` из `[4:8]` и парсит столько байт JSON (пробелы
в хвосте игнорируются), корректно разбирает оба варианта. Поля `serial`/`calibration`
присутствуют только если прибор их сообщил; у сегмента `saved_rows=0` означает, что он
