...
`seg_evicted` растёт (безопасно — см. ниже); кольцо PSRAM и WS-стрим продолжают работать
без перерыва.
...
| Flash заполнен | **кольцо keep-last**: затирается самый старый ещё не отправленный сегмент; `flash_full=true`, растёт `seg_evicted` (безопасно) |
...
- `seg_lost` — **#FW-57** (2026-08-19), реальная потеря: boot-реконсиляция нашла сегмент с
  size=0 (данные могли физически быть на flash, но inode не подтвердил размер — потеря
  питания/аварийный ребут, см. `KNOWN_ISSUES.md` P-016). Накопительный, переживает ребут
  (NVS), сбрасывается только `/api/waterfall/clear`. **Раньше это событие и безопасное
  вытеснение кольцом считались в одно поле `seg_dropped` — различить рост было нельзя**;
- `seg_evicted` — **#FW-56**, сегментов затёрто кольцом keep-last (безопасно: сегмент уже
  был скачан или устарел). Тоже накопительный (NVS), тоже переживает ребут;
- `seg_dropped` — **устарело (deprecated), сохранено для совместимости**: отдаётся как
  алиас `seg_lost`. Поле публичное, его читают внешние приёмники и скрипты — молча убрать
  его значило бы сломать их без предупреждения. Алиас указывает именно на `seg_lost`
  (тревожная метрика), а не на сумму: клиент, следивший за «что-то потерялось», должен и
  дальше видеть потери, а не штатную ротацию. **В новых интеграциях использовать
  `seg_lost` и `seg_evicted`**; поле будет удалено не раньше следующей мажорной версии.
...
- **`#FW-60/#FW-61`**: листинг берётся из RAM-реестра (обновляется в точках изменения
  `seg_count`), а не обходом каталога — раньше давал до 1550 мс на четыре сегмента, теперь
  LIVE-класс без блокировки;
- `bytes` — фактический размер файла (`stat()`), не вычисляется формулой: у сегментов
  старых версий формата другой `row_stride` и может не быть секции baseline, формула по
  геометрии текущей версии для них ошибалась;
- `rows` — прямое поле реестра, обновляется по ходу записи строк (не только на
  финализации), внешняя трассировка ролловера видит рост, а не константу;
- `finalized:false` — сегмент сейчас открыт — забирать не нужно. С `firmware-v1.2.17`
  (`#FW-63`) такой сегмент переживает и внезапный сброс питания: метаданные сбрасываются
  на носитель не реже раза в минуту, поэтому даже незакрытый сегмент восстанавливается
  реконсиляцией при загрузке вместо потери целиком;
- каталога ещё нет / запись не велась → `[]`.

## Мониторинг CPS (`#MON-1`, `firmware-v1.2.16+`)

Вкладка «Мониторинг» показывает скорость счёта во времени независимо от записи водопада.
Архитектура: плата сама, автономно и непрерывно копит односекундные базовые отсчёты в
кольце PSRAM (**12 ч** с `firmware-v1.2.16`, `#MON-3`; до этого — 6 ч) с монотонным номером
`seq`. Сбор не требует открытой страницы и не останавливается при закрытии вкладки —
кольцо и есть источник истины, вкладка лишь читает его хвост.

| Эндпоинт | Метод | Что делает |
|---|---|---|
| `/monitor` | GET | Web UI мониторинга |
| `/api/monitor/series?since=<seq>` | GET | Хвост серии начиная с `seq+1`. Read-only, CSRF не требуется |
...
- `epoch` — эпоха серии; меняется при сбросе спектра/прибора или перезапуске платы
  (кольцо теряется). Клиент, увидевший новый `epoch`, обязан считать локальную историю
  недействительной и начать ресинк с `first_seq`;
- `first_seq` / `next_seq` — границы того, что сейчас доступно в кольце;
  `since=4294967294` (заведомо за пределом) — дешёвый способ только зондировать курсоры,
  не запрашивая сами отсчёты;
- `samples` — до 2000 элементов за один ответ (`MON_SERIES_CHUNK`); клиент, которому нужно
  больше, дотягивает историю циклом запросов, пока не догонит `next_seq`;
- каждый отсчёт `[end_sec, dur, counts]` — наборное время прибора на конец интервала,
  длительность интервала (с) и прирост числа импульсов за него. Клиент агрегирует базовые
  секундные отсчёты в «вёдра» по выбранному интервалу усреднения самостоятельно — плата
  отдаёт только сырьё.

Веб-страница восстанавливает всю доступную историю при открытии без активной сессии
записи (до `firmware-v1.2.16` показывала только то, что накоплено после нажатия «Старт» —
дефект UI, не прошивки: плата собирала данные и раньше, страница их не забирала).
...
**`started_at` (якорь сегмента) — ДРУГОЙ источник времени, чем `dur`.** `dur`
каждой строки берётся с живых часов ПРИБОРА (`total_time_sec`) и от интернета
никак не зависит — реляционные интервалы между строками честны всегда. А вот
`started_at` в шапке сегмента = `time(NULL)` платы (ESP32 wall clock), который
синкается по SNTP (`pool.ntp.org`, `init_sntp()`, только исходящий запрос при
старте, без ретрая/проверки успеха). **Если у платы никогда не было интернета —
`time(NULL)` не синкан, `started_at` близок к эпохе** (наблюдался случай
`started_at=1`). Автономная запись при этом идёт нормально — теряется только
АБСОЛЮТНАЯ привязка ко времени (дата в шапке / экспорт N42), сами длительности
строк (`dur`) остаются реальными. Якорь берётся заново при открытии КАЖДОГО
нового сегмента — если интернет появился посреди записи, сегменты до и после
получат разные (несогласованные) `started_at`, а скачок между ними НЕ
детектируется как «пауза платы» (см. `wf_pull_client.py`, детекция gap явно
отключена при `started_at ≤ 1e9`).
