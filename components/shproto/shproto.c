...
void shproto_byte_received(shproto_struct *p, uint8_t byte)
{
    ...
    if (byte == SHPROTO_FINISH) {
        if (p->len >= 3) {
            uint16_t crc = crc16_buf(p->data, p->len);
            if (crc == 0) {
                p->cmd = p->data[0];
                p->len -= 3;
                if (p->len > 0)
...
static void add_escaped(shproto_struct *p, uint8_t byte)
{
    if (byte == SHPROTO_START || byte == SHPROTO_FINISH || byte == SHPROTO_ESC) {
        if (p->len < p->buf_size) p->data[p->len++] = SHPROTO_ESC;
        byte = (~byte) & 0xFF;
    }
    if (p->len < p->buf_size) p->data[p->len++] = byte;
}
...
void shproto_packet_start(shproto_struct *p, uint8_t cmd)
{
    p->len    = 0;
    p->crc_tx = CRC_INIT;
    if (p->len < p->buf_size) p->data[p->len++] = 0xFF;
    if (p->len < p->buf_size) p->data[p->len++] = SHPROTO_START;
    p->cmd = cmd;
    p->crc_tx = crc16_byte(p->crc_tx, cmd);
    add_escaped(p, cmd);
}

void shproto_packet_add_data(shproto_struct *p, uint8_t byte)
{
    p->crc_tx = crc16_byte(p->crc_tx, byte);
    add_escaped(p, byte);
}

void shproto_packet_complete(shproto_struct *p)
{
    uint16_t crc = p->crc_tx;
    add_escaped(p, crc & 0xFF);
    add_escaped(p, crc >> 8);
    if (p->len < p->buf_size) p->data[p->len++] = SHPROTO_FINISH;
}